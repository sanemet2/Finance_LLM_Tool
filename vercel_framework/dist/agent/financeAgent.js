...
if (!apiKey) {
    throw new Error(`Environment variable ${OPENROUTER_API_KEY_ENV} must be set to call the finance agent.`);
}
const resolvedApiKey = apiKey;
// Main entrypoint: runs the chat loop, wiring model responses to tool executions until completion.
export async function generateFinanceAnswer(prompt) {
    const messages = [
    ...
    while (true) {
        ...
        for (const call of toolCalls) {
            console.log(`[tool] ${call.function?.name ?? "unknown"} args: ${call.function?.arguments ?? "{}"}`);
